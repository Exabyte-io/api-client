...
class EndpointMaterialsUnitTest(EndpointBaseUnitTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.endpoint = ExabyteMaterialsEndpoint(self.host, self.port, self.user_id, self.auth_token)

    @mock.patch('requests.sessions.Session.request')
    def test_get_materials(self, mock_request):
        mock_request.return_value = self.mock_response('[]')
        self.assertEqual(self.endpoint.get_materials(), [])
        self.assertEqual(mock_request.call_args[1]['method'], 'get')

    @mock.patch('requests.sessions.Session.request')
    def test_get_material(self, mock_request):
        mock_request.return_value = self.mock_response('{}')
        self.assertEqual(self.endpoint.get_material('28FMvD5knJZZx452H'), {})
    ...
    def test_get_material_by_formula(self, mock_request):
        mock_request.return_value = self.mock_response('[]')
        self.assertEqual(self.endpoint.get_materials_by_formula('Si'), [])
    ...
    def test_create_material(self, mock_request):
        ...
        self.endpoint.create_material(self.get_content_in_json('new-material.json'))
